...
import com.fubaotv.vo.FubaoUserVo;
import com.fubaotv.vo.StreamingVo;
...
public class MenuListDao {
    ...
    public void MovieMenu(List<FubaoUserVo> list) {
        System.out.println("반갑습니다 ." + list.get(0).getNick() + "회원님 원하시는 메뉴를 선택해주세요");
        ...
        switch (num) {
            case 1: case 2: case 3: case 4:
                ...
                MovieChoice(list.get(0).getNick(), s_list.get(0).getSNum(), s_list.get(0).getViews());
                MovieMenu(list);
                break;
            case 5:
                System.out.println("광고가 재생 됩니다.");
                aduserDao.BannerSelect(); // 광고 재생 매소드
                fubaoUserDao.tokenUP(list.get(0).getNick()); // 토큰 1개 추가
                MovieMenu(list);
                break;
            case 6:
                fubaoUserDao.signUpView(list); // 회원정보 뷰어
                UserChange(list); //회원정보 변경
                MovieMenu(list);
                break;
            case 7:
    ...
    public void UserChange(List<FubaoUserVo> list) {
        ...
        switch (num) {
            case 1:
                ...
                u_list=fubaoUserDao.Change_id(list.get(1).getId());
